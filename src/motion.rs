...
/// Полный оборот дрейфа подложки.
///
/// Значение согласовано со спецификацией движения, а вот самого вечного
/// дрейфа у Savio пока нет: подложка только перекладывается к выбранному
/// разделу и останавливается. Причина в цене — пересобирать сетку фона в
/// каждом кадре значило бы просить кадры в покое, единственной анимации во
/// всём наборе (задача 56 реестра). Число оставлено здесь, чтобы тот, кто
/// возьмётся за задачу, не выбирал его заново.
#[allow(dead_code)]
pub const DRIFT: f32 = 19.0;
