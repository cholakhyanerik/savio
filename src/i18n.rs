...
strings! {
    ...
    // Экран приветствия при первом запуске. Он рассказывает обо всём Savio,
    // а не об одной загрузке: пять разделов из шести прежде не были названы
    // в нём вовсе, и узнавали о них только те, кто перещёлкал рельс.
    //
    // Заметно изменив рассказ, поднимите `WELCOME_EDITION` в app.rs: без
    // этого новое приветствие увидят только новые люди, а все, кто закрыл
    // прежнее, так и останутся со старым представлением о программе.
    WelcomeTitle =
        "Шесть инструментов в одном окне",
        "Six tools in one window",
        "Վեց գործիք մեկ պատուհանում";
    WelcomeNote =
        "Разделы не зависят друг от друга: можно пользоваться одним и ни разу не открыть \
         остальные. Щёлкните по разделу — он откроется.",
        "The sections do not depend on each other: you can use one and never open the rest. \
         Click a section and it opens.",
        "Բաժիններն իրարից կախված չեն՝ կարելի է օգտվել մեկից և երբեք չբացել մնացածները։ \
         Սեղմեք բաժնի վրա, և այն կբացվի։";
    // Строка «что это даёт» под названием раздела в приветствии — по одной на
    // пункт рельса (`NavItem::pitch`). Названия разделов здесь не повторяются:
    // они стоят над строкой и берутся теми же ключами, что в рельсе.
    //
    // У «Загрузки» строка пересказывает прежние три шага: сам экран загрузки
    // и так собран в пронумерованные шаги, и повторять их здесь незачем.
    //
    // У «Сейчас» о питании ни слова намеренно: схема и «Режим питания»
    // переключаются только в Windows, а приветствие одно на три системы.
    WelcomeDownloadPitch =
        "Видео и звук по ссылке: вставьте её и нажмите «Скачать».",
        "Video and audio by link: paste it and press “Download”.",
        "Տեսանյութ և ձայն հղումով՝ տեղադրեք այն և սեղմեք «Ներբեռնել»։";
    WelcomeMetadataPitch =
        "Что фото и MP3 рассказывают о вас — и как это стереть.",
        "What your photos and MP3s tell about you — and how to wipe it.",
        "Ինչ են լուսանկարներն ու MP3-ները պատմում ձեր մասին, և ինչպես դա ջնջել։";
    WelcomeMachineNowPitch =
        "Нагрузка на компьютер: процессор, память, диски, сеть.",
        "The computer's load: processor, memory, disks, network.",
        "Համակարգչի բեռնվածությունը՝ պրոցեսոր, հիշողություն, սկավառակներ, ցանց։";
    WelcomeMachineSpecPitch =
        "Из чего собран компьютер и в каком он состоянии.",
        "What the computer is made of and what shape it is in.",
        "Ինչից է բաղկացած համակարգիչը և ինչ վիճակում է այն։";
    WelcomeWeatherPitch =
        "Что за окном сейчас, по часам и на неделю.",
        "What is outside now, by the hour and for the week.",
        "Ինչ է դրսում հիմա, ժամերով և շաբաթվա համար։";
    WelcomePhonePitch =
        "Файлы между телефоном и компьютером по своей сети Wi-Fi.",
        "Files between the phone and the computer over your own Wi-Fi.",
        "Ֆայլեր հեռախոսի և համակարգչի միջև ձեր Wi-Fi ցանցով։";
    ...
    UiAboutButtonsNote =
        "«Показать приветствие» снова откроет обзор всех разделов — тот же, что при первом \
        ...
        "“Show the welcome screen” opens the overview of all sections again — the same as on \
         first run. “Write” opens the mail program with the version number in the subject; if it \
         is not set up, “Copy the address” helps.",
        "«Ցույց տալ ողջույնը» կրկին կբացի բոլոր բաժինների ակնարկը՝ նույնը, ինչ առաջին \
         գործարկման ժամանակ։ «Գրել»-ը կբացի փոստային ծրագիրը՝ նամակի թեմայում տարբերակի \
         համարով. եթե այն կարգավորված չէ, կօգնի «Պատճենել հասցեն»-ը։";
    UiShowWelcome = "Показать приветствие", "Show the welcome screen", "Ցույց տալ ողջույնը";
    UiHelpButtonHint =
        "Открыть приветствие — обзор всех разделов, как при первом запуске.",
        "Open the welcome screen — an overview of all sections, as on first run.",
        "Բացել ողջույնի էկրանը՝ բոլոր բաժինների ակնարկը, ինչպես առաջին գործարկման ժամանակ։";
