...
/// Затемнение под модальным окном установки. Полупрозрачное, а не сплошное:
/// главное окно должно просвечивать, иначе модалка выглядит отдельным
/// приложением, а не слоем поверх Savio. Плотность подобрана так, чтобы
/// текст под ней не читался и не отвлекал.
pub const MODAL_BACKDROP: Color32 = Color32::from_black_alpha(180);
...
/// Журнал, подсказка в пустом поле и версия в шапке. На `BG_SURFACE` даёт
/// 5.74:1 — порог 4.5:1 проходит. На `BG_PRESSED` уже не проходит, поэтому
/// на нажатых поверхностях не применяется.
...
/// всего 16 точек в поперечнике, и при `RADIUS_SMALL` она скругляется почти
/// в кружок — то есть выглядит радиокнопкой, элементом с другим смыслом
/// («одно из»), хотя галочки независимы.
pub const RADIUS_TINY: u8 = 4;
