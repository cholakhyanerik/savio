...
pub struct Settings {
    ...
    /// второй — заголовку свёрнутой группы. Отсюда требование: заголовок
    /// обязан отличать включённый вход от выключенного взглядом, а не
    /// вычитыванием, и отличает жёлтой точкой (задача 51 реестра) — слова
    /// у заглушки те же. Без этого запомненный вход становится невидимым,
    /// а он хрупкий: у YouTube cookies ломают загрузку совсем, при открытом
    /// браузере база занята.
