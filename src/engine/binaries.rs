...
/// Имя файла-метки «приветствие уже показывали».
const WELCOME_MARK: &str = ".welcome-shown";

/// Показывали ли приветствие этого издания или более нового.
///
/// Файлом рядом с инструментами, а не полем в `settings.json`, и это выбор
/// человека, а не следствие устройства. Разница в том, что настройки человек
/// правит и переносит сам, а эта метка — не настройка: она ничего не
/// выбирает и ничего не значит, кроме «здесь уже были».
///
/// Издание — номер содержания приветствия, и выбирает его окно, а не движок:
/// здесь он только хранится. Заведено оно затем, чтобы приветствие, заметно
/// изменившее рассказ, один раз показалось и тем, кто закрыл прежнее.
/// «Не меньше», а не «равно»: запущенная после новой старая версия Savio
/// своего приветствия поверх уже виденного нового не покажет.
///
/// **Не удалось определить папку или прочитать метку — считаем, что не
/// показывали.** Приветствие лишний раз покажется, и это неприятно; не
/// показаться вовсе оно не должно: оно и есть объяснение того, как
/// пользоваться программой.
pub fn welcome_seen(edition: u32) -> bool {
    let Some(dir) = data_dir() else {
        return false;
    };
    std::fs::read_to_string(dir.join(WELCOME_MARK))
        .is_ok_and(|mark| edition_in_mark(&mark) >= edition)
}

/// Какое издание приветствия записано в метке.
///
/// Пустая метка — первое издание: до издания с номером её писали пустой,
/// и все, у кого она такая, видели приветствие о трёх шагах загрузки.
/// Нечитаемое содержимое считается тем же первым изданием: метка есть,
/// значит, какое-то приветствие здесь уже видели.
fn edition_in_mark(mark: &str) -> u32 {
    mark.trim().parse().unwrap_or(1)
}

/// Отмечает, что приветствие этого издания показали.
///
/// Все неудачи молчаливые, как и у настроек: не создалась папка, диск полон,
/// каталог только для чтения — всё это значит лишь, что приветствие
/// покажется ещё раз. Ругаться на это нельзя, и падать тем более.
pub fn mark_welcome_seen(edition: u32) {
    let Some(dir) = data_dir() else {
        return;
    };
    let _ = std::fs::create_dir_all(&dir);
    let _ = std::fs::write(dir.join(WELCOME_MARK), edition.to_string());
}
...
mod tests {
    ...
    /// Метка прежних версий — пустой файл, и читается она как первое издание.
    ///
    /// Ровно на этом держится повторный показ: прочитайся пустая метка как
    /// «ничего не видели» или как «видели всё», новое приветствие либо
    /// показывалось бы при каждом запуске, либо не показалось бы тем, ради
    /// кого его и показывают ещё раз.
    ///
    /// Проверено красным: с нулём вместо единицы падает на пустой метке.
    #[test]
    fn a_welcome_mark_tells_which_edition_was_seen() {
        assert_eq!(edition_in_mark(""), 1, "пустая метка прежних версий");
        assert_eq!(edition_in_mark("2"), 2);
        assert_eq!(edition_in_mark("2\n"), 2, "перевод строки от правки руками");
        assert_eq!(edition_in_mark("что-то"), 1, "метка есть — что-то видели");
        // То, что пишет `mark_welcome_seen`, читается обратно тем же числом.
        assert_eq!(edition_in_mark(&7_u32.to_string()), 7);
    }
}
