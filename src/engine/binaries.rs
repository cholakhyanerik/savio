...
/// Показывали ли приветствие хоть раз.
///
/// Пустым файлом рядом с инструментами, а не полем в `settings.json`, и это
/// выбор человека, а не следствие устройства. Разница в том, что настройки
/// человек правит и переносит сам, а эта метка — не настройка: она ничего
/// не выбирает и ничего не значит, кроме «здесь уже были».
///
/// **Не удалось определить папку — считаем, что не показывали.** Приветствие
/// лишний раз покажется, и это неприятно; не показаться вовсе оно не должно:
/// оно и есть объяснение того, как пользоваться программой.
pub fn welcome_seen() -> bool {
    data_dir().is_some_and(|dir| dir.join(WELCOME_MARK).exists())
}

/// Отмечает, что приветствие показали.
///
...
pub fn mark_welcome_seen() {
    ...
    let _ = std::fs::write(dir.join(WELCOME_MARK), b"");
}
